...
RUNFILE=plot-asr-fin-sweep1.py

## create/overwrite directory to store results
mkdir -p $PBS_O_WORKDIR/plot-asr-fin-sweep1

## Copy and move to space for running job:
cp $PBS_O_WORKDIR/$RUNFILE $TMPDIR 
cp $PBS_O_WORKDIR/sel-asr-fin-sweep1/*.npz $TMPDIR
...
cd $TMPDIR
cp ./*.png $PBS_O_WORKDIR/plot-asr-fin-sweep1
