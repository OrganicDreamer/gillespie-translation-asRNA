import gillespie1mR.compl_transl_stat as ct
import gillespie1mR.occ_dist_stat as mrna
import gillespie1mR.static as gillsim
import gillespie1mR.sequestered as strand
import numpy as np
...
for i in range(sweep_kinetic_const.shape[0]):
    ...
    (trace_mrna, trace_ribo, trace_asrna, trace_time, steady_mrna, steady_ribo, steady_asrna, steady_time,
     kinetic_rates) = gillsim.stat_pool(initial_mrna, free_ribo, free_asrna, 0, window_start_time,sweep_kinetic_const[i][0],sweep_kinetic_const[i][1],sweep_kinetic_const[i][2],sweep_kinetic_const[i][3])
    ...
    (steadytrace_mrna, steadytrace_ribo, steadytrace_asrna, steadytrace_time, final_mrna, final_ribo, final_asrna,
     final_time, steadykinetic_rates) = gillsim.stat_pool(steady_mrna, steady_ribo, steady_asrna, steady_time,
    ...
    strand.ribo_seq(steadytrace_time,steadytrace_mrna,final_mrna,steadykinetic_rates)